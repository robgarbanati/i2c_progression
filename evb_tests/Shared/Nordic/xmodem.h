...
#define XMODEM_ERROR_FILEIO       (-5)

// Send/recv functions
typedef void (*p_xm_start_func)(void);
typedef void (*p_xm_done_func)(void);
typedef void (*p_xm_send_func)(uint8_t);
typedef int (*p_xm_recv_func)(uint32_t);

//! xmodem receive
long xmodem_receive(SPIFSFile *file);

//! initialize xmodem management callback routines
