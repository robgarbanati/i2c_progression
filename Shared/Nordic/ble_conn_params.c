...
// Timeout timer.
static void update_timeout_handler(void const *arg);
osTimerId m_conn_params_timer_id;
osTimerDef(m_conn_params_timer, update_timeout_handler);

// Ble connection mutex.
osMutexId m_ble_conn_mutex;
osMutexDef(m_ble_conn_mutex);
...
static void update_timeout_handler(void const *p_arg)
{
    UNUSED_PARAMETER(p_arg);

		osMutexWait(m_ble_conn_mutex, osWaitForever);
    ...
    if (m_conn_handle != BLE_CONN_HANDLE_INVALID)
    {
		...
		osMutexRelease(m_ble_conn_mutex);
}
...
uint32_t ble_conn_params_init(const ble_conn_params_init_t * p_init)
{
    ...
    m_update_count = 0;
		...
		m_ble_conn_mutex = osMutexCreate(osMutex(m_ble_conn_mutex));

		m_conn_params_timer_id = osTimerCreate(osTimer(m_conn_params_timer), osTimerOnce, NULL);
...
{
		osTimerStop(m_conn_params_timer_id);
    return NRF_SUCCESS;
}
        ...
        else
        {
            ...
            else
            {
                timeout_ticks = m_conn_params_config.next_conn_params_update_delay;
						...
						osTimerStart(m_conn_params_timer_id, timeout_ticks);
        }
    }
    else
    {
